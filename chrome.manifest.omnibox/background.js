...
let nameSpace = chrome || browser;
nameSpace.tabs = undefined;

// Provide help text to the user.
nameSpace.omnibox.setDefaultSuggestion({
  description: `Search the firefox codebase
    (e.g. "hello world" | "path:omnibox.js onInputChanged")`
});

// Update the suggestions whenever the input is changed.
nameSpace.omnibox.onInputChanged.addListener((text, addSuggestions) => {
  console.log('text:', text);
  console.log('addSuggestions:', addSuggestions);
  let headers = new Headers({"Accept": "application/json"});
  let init = {method: 'GET', headers};
  let url = buildSearchURL(text);
  console.log('get guild search url:', url)
...
nameSpace.omnibox.onInputEntered.addListener((text, disposition) => {
  let url = text;
  ...
  switch (disposition) {
    case "currentTab":
      nameSpace.tabs.update({url});
      break;
    case "newForegroundTab":
      nameSpace.tabs.create({url});
      break;
    case "newBackgroundTab":
      nameSpace.tabs.create({url, active: false});
      break;
  }
});
