...
document.querySelector('#start').addEventListener('click', function (event) {
    pending_request_id = chrome.desktopCapture.chooseDesktopMedia(DESKTOP_MEDIA, onAccessApproved);
});
...
document.querySelector('#startFromBackgroundPage').addEventListener('click', function (event) {
        chrome.runtime.sendMessage({type: 'startCapture',}, function (response) {
                console.log('get streamId from background page:', response.streamId);
               if(response.streamId){
                   onAccessApproved(response.streamId)
               }
            });
    });
...
function onCreateOfferSuccess(desc) {
    ...
    pc2.createAnswer(onCreateAnswerSuccess, function () {
    }, sdpConstraints);
}
